import { Config, Context, Params } from "../types";
...
export function addBefore(ctx: Context) {
    let s = ctx.addBefore.join("\n");
    ctx.addBefore = [];
    return s;
}

export function createUniqueTypeName(ctx: Context) {
    return `${path2LCamal(ctx.path)}_${ctx.param}_${randomUUID().slice(0, 4)}`
}